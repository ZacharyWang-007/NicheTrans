...
def train(args, model, criterion, optimizer, trainloader, panel=None, img_information=True):
    ...
    for batch_idx, (imgs, rna, msi, rna_neighbors, _, _) in enumerate(trainloader):

        imgs, rna, msi, rna_neighbors = imgs.cuda(), rna.cuda(), msi.cuda(), rna_neighbors.cuda()
        ...
        source, target, source_neightbors = rna, msi, rna_neighbors

        if img_information ==True:
...
def test(args, model, testloader, best_pearson, panel=None, last_epoch=False, img_information=True):
    ...
    with torch.no_grad():
        for _, (imgs, rna, msi, rna_neighbors, _, samples) in enumerate(testloader):
